...
int32_t Graph_Select_W(GraphRef Graph__, uint32_t pos__, Graph_value val__) {
  ...
  switch (val__) {
    ...
    case VALUE_Tx:
      temp = graph_select_masked_(*Graph__, pos__, false, VECTOR_W6);

      if (Graph__->dpos_ == DPOS_NOT_PRESENT ||
          temp <= (int32_t) Graph__->dpos_)
        return temp;

      return graph_select_masked_(*Graph__, pos__ + 1, false, VECTOR_W6);
    ...
    case VALUE_Tx:
      ...
      temp = graph_select_masked_(*Graph__, pos__ + 1, false, VECTOR_W6);
      temp = graph_select_masked_(*Graph__, temp, false, VECTOR_W2);
      return graph_select_simple_(*Graph__, temp, false, VECTOR_W0);
