...
int32_t graph_Wselect_masked_(Graph_Struct Graph__, uint32_t num__, bool zero__, int32_t type__) {
  ...
  /* gets optimized away if all expansions are successfull */
  assert(type__ >= VECTOR_W1 && type__ <= VECTOR_W6);
...
int32_t Graph_Select_W(GraphRef Graph__, uint32_t pos__, Graph_value val__) {
  ...
  switch (val__) {
    ...
    case VALUE_Tx:
      temp = graph_Wselect_masked_(*Graph__, pos__, false, VECTOR_W6);

      if (Graph__->dpos_ == DPOS_NOT_PRESENT ||
          temp <= (int32_t) Graph__->dpos_)
        return temp;

      return graph_Wselect_masked_(*Graph__, pos__ + 1, false, VECTOR_W6);
    ...
    case VALUE_Tx:
      ...
      temp = graph_Wselect_masked_(*Graph__, pos__ + 1, false, VECTOR_W6);
      temp = graph_Wselect_masked_(*Graph__, temp, false, VECTOR_W2);
      return graph_Wselect_simple_(*Graph__, temp, false, VECTOR_W0);
